...
public class ConversionActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        res = getResources();
        String[] currArry = res.getStringArray(R.array.flags);

        if (extras != null) {
            CRYPTO_POSITION = extras.getInt("crypto_position");
            if (Integer.parseInt(sharedPref.getString("List_" + CRYPTO_POSITION, null).split("#")[2]) == (R.drawable.btc_icon)) {
                CRYPTO_TYPE = "BTC";
                base_currency.setHint("1 " + CRYPTO_TYPE);
            } else if (Integer.parseInt(sharedPref.getString("List_" + CRYPTO_POSITION, null).split("#")[2]) == (R.drawable.eth)) {
                CRYPTO_TYPE = "ETH";
                base_currency.setHint("1 " + CRYPTO_TYPE);
            }
